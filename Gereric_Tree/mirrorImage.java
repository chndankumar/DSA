...
public class mirrorImage {
    ...
    public static void main(String[] args) {
        ...
        int d = distance(root, 70, 110);
        Node root1 = new Node(d, null);
        boolean mirrorImg = isMirrorImage(root, root1);
    }
    ...
    private static boolean isMirrorImage(Node root, Node root1) {
        if (root.children.size() != root1.children.size()) {
            return false;
        }
        for (int i = 0; i < root.children.size(); i++) {
            int n = root1.children.size() - 1;
            if (isMirrorImage(root.children.get(i), root1.children.get(n - i)) == false) {
                return false;
            }
        }
        return true;
    ...
    private static int distance(Node root, int i, int j) {
        ArrayList<Node> list1 = getPath(root, i);
        ArrayList<Node> list2 = getPath(root, j);
        int s1 = list1.size();
        int s2 = list2.size();
        while (s1 > 0 && s2 > 0 && list1.get(s1 - 1) == list2.get(s2 - 1)) {
            list1.remove(s1 - 1);
            list2.remove(s2 - 1);
            s1--;
            s2--;
